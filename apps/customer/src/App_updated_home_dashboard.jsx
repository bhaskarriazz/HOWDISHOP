...
function App() {
  ...
  const [customerLocation, setCustomerLocation] = useState(() => {
    try {
      return localStorage.getItem("howdiCustomerLocation") || "";
    } catch {
      return "";
    }
  });
...
return () => window.clearInterval(timer);
  ...
  const placeCheckoutOrder = () => {
    ...
    const newOrder = {
      ...
      title: cart.length === 1 ? cart[0].name : `${cart[0]?.name || "HOWDI"} + ${cart.length - 1} more`,
      shop: cart[0]?.shop || "HOWDI Marketplace",
      total: `₹${checkoutTotal.toLocaleString("en-IN")}`,
      amount: checkoutTotal,
      item_count: cart.reduce((sum, item) => sum + (item.quantity || 1), 0),
      items: cart,
      status: "confirmed",
      ...
      tracking_number: `HOWDI${String(Date.now()).slice(-10)}`,
  ...
  // ==============================
  // Customer locations are controlled by the Admin Location List.
  // Legacy string entries are normalized so existing admin data continues
  // to work while newer entries can include PIN codes and metadata.
    ...
    try {
      ...
      const normalized = saved
        .map((item) => {
          ...
          const city = String(
            item.city || item.name || item.location || ""
          ).trim();

          if (!city) return null;

          const pincodes = Array.isArray(item.pincodes)
            ? item.pincodes
                .map((pin) =>
                  String(pin).replace(/\D/g, "").slice(0, 6)
                )
      ...
      // Prevent duplicate customer choices for the same city.
      const unique = [];
      const seen = new Set();
      ...
      console.error(
        "Unable to read HOWDI service locations:",
        error
      );
      return [];
    }
  ...
  const availableLocationObjects = getAvailableLocations();

  const availableLocations = availableLocationObjects.map(
    (location) => location.city
  );

  const filteredLocations = availableLocationObjects.filter(
    (location) =>
      location.city
        .toLowerCase()
  ...
  // Restore only an enabled/admin-approved location.
  // If the old saved city has been disabled or removed by Admin,
  // it is cleared instead of being treated as serviceable.
  useEffect(() => {
    ...
    if (!locations.length) {
      ...
      try {
        localStorage.removeItem("howdiCustomerLocation");
        localStorage.removeItem("howdiCustomerLocationData");
      } catch {}
    ...
    const saved = localStorage.getItem(
      "howdiCustomerLocation"
    );

    const savedLocation = locations.find(
      (location) =>
        location.city.toLowerCase() ===
        String(saved || "").trim().toLowerCase()
    );

    if (savedLocation) {
      setCustomerLocation(savedLocation.city);

      try {
        localStorage.setItem(
          "howdiCustomerLocationData",
    ...
    // No valid saved selection: use the first enabled
    // admin-approved location.
    const firstLocation = locations[0];

    setCustomerLocation(firstLocation.city);
  ...
  const chooseCustomerLocation = (location) => {
    ...
    const value = String(selected.city || "").trim();
    if (!value) return;
    ...
    try {
      ...
      // Keep the full location record available to Search,
      // Product and Checkout for later delivery validation.
      localStorage.setItem(
        "howdiCustomerLocationData",
      ...
      console.error(
        "Unable to save HOWDI customer location:",
        error
      );
    }
  ...
  // ==============================
  // HOME DASHBOARD DATA / LOCATION AWARENESS
  // ==============================
  // Keep the existing Home UI, but make its discovery sections respond to
  // the selected Admin-approved customer location where the available data
  // provides a location/service-area field. Products without such metadata
  // remain visible so existing catalog data is not silently hidden.
  const getLocationTokens = (locationRecord, fallbackCity = "") => {
    ...
    const values = [
      ...
      ...(Array.isArray(locationRecord.pincodes) ? locationRecord.pincodes : []),
      ...(Array.isArray(locationRecord.pinCodes) ? locationRecord.pinCodes : []),
    ];
    ...
    return values
      ...
      .concat(fallbackCity ? [String(fallbackCity).trim().toLowerCase()] : []);
  };

  const selectedCustomerLocationData = (() => {
    try {
      const saved = JSON.parse(
        localStorage.getItem("howdiCustomerLocationData") || "null"
      );
  ...
  const selectedLocationTokens = getLocationTokens(
    selectedCustomerLocationData,
    customerLocation
  );

  const valueContainsSelectedLocation = (value) => {
    if (!value || !selectedLocationTokens.length) return false;
  ...
  const isItemAvailableForCustomerLocation = (item) => {
    if (!item || !selectedLocationTokens.length) return true;

    // If an item has explicit location/service-area metadata, honor it.
    const explicitLocationFields = [
      ...
      item.deliveryCities,
      item.delivery_cities,
      item.deliveryPincodes,
      item.deliveryPincodes,
      item.pincodes,
      item.pinCodes,
    ].filter(Boolean);

    if (!explicitLocationFields.length) {
      // Existing demo/catalog records have no service-area metadata.
      // Keep them visible until Admin/product location mapping is added.
      return true;
    }

    return valueContainsSelectedLocation(explicitLocationFields);
  };

  const getHomeProducts = () =>
    Array.isArray(products)
      ? products.filter(isItemAvailableForCustomerLocation)
      : [];

  const getHomeWorkers = () =>
    Array.isArray(workers)
      ? workers.filter(isItemAvailableForCustomerLocation)
      : [];

  // Recently viewed is already written by the product-detail flow.
  // Normalize it here so Home can safely render only valid catalog records.
  const getRecentlyViewedProducts = () => {
    try {
      const saved = JSON.parse(
        localStorage.getItem("howdiRecentlyViewed") || "[]"
      );
  ...
  const homeWorkers = getHomeWorkers();
  const recentlyViewedProducts = getRecentlyViewedProducts();

  // Lightweight recommendation ranking based on customer activity already
  // present in localStorage: wishlist + recently viewed + catalog metadata.
  const getRecommendedProducts = () => {
    const catalog = [...homeProducts];

    let wishlistItems = [];
    try {
      const saved = JSON.parse(
        localStorage.getItem("howdiWishlist") || "[]"
      );
      wishlistItems = Array.isArray(saved) ? saved : [];
    } catch {}

    const recentNames = new Set(
      recentlyViewedProducts.map((item) => item.name)
    );
    const wishlistNames = new Set(
      wishlistItems.map((item) => item?.name).filter(Boolean)
    );
    ...
    return catalog
      .map((product, index) => {
        ...
        if (recentNames.has(product.name)) score += 5;
        if (wishlistNames.has(product.name)) score += 4;
        if (product.handmade) score += 2;
        if (Number(product.rating || 0) >= 4.8) score += 2;
        if (Number(product.reviews || 0) >= 50) score += 1;
            ...
            <div style={{ marginTop: "14px", display: "grid", gap: "8px", maxHeight: "280px", overflowY: "auto" }}>
              {filteredLocations.length ? filteredLocations.map((location) => (
                <button
                  ...
                  style={{ width: "100%", textAlign: "left", border: location.city === customerLocation ? "2px solid #365947" : "1px solid #e2e8f0", borderRadius: "12px", padding: "12px 13px", background: location.city === customerLocation ? "#f0f7f2" : "#fff", color: "#172033", cursor: "pointer", fontWeight: 800, display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}
                >
                  <span>
                    <span style={{ display: "block" }}>📍 {location.city}</span>
                    {Array.isArray(location.pincodes) && location.pincodes.length > 0 && (
                      <small style={{ display: "block", marginTop: "3px", color: "#64748b", fontWeight: 600 }}>
                        {location.pincodes.length} service PIN code{location.pincodes.length === 1 ? "" : "s"}
                      </small>
                    )}
                  </span>
                  {location.city === customerLocation && <span style={{ color: "#365947", whiteSpace: "nowrap" }}>✓ Selected</span>}
                </button>
              )) : (
      ...
      {cartOpen && (
        ...
        >
          ...
          >
            ...
            >
              <div>
                ...
                <h2 style={{ margin: "5px 0 0", fontSize: "25px" }}>
                  🛒 Your Cart
                  {savedForLater.length > 0 && <span style={{ fontSize: "12px", color: "#365947", marginLeft: "8px" }}>· {savedForLater.length} saved</span>}
                </h2>
              </div>
            ...
            <div style={{ flex: 1, minHeight: 0, overflowY: "auto", overscrollBehavior: "contain", WebkitOverflowScrolling: "touch", padding: "20px 20px 8px" }}>
              {cart.length === 0 ? (
                <div
                  style={{
                    minHeight: "360px",
                      ...
                      <div>
                        ...
                        {(item.selectedSize || item.size || item.selectedColor || item.color || item.customMeasurements || item.fitSizeResult) && (
                          <div style={{
                            ...
                            <strong>🧶 Your handmade customization</strong>
                            ...
                            {item.fitPhotoPermission ? <div>🔒 Fit reference: permission granted · image not shown here</div> : null}
                          </div>
                        )}

                        {item.makingTime && (
                          <div style={{ marginTop: "7px", color: "#64748b", fontSize: "11.5px" }}>
                            🧵 Making time: {item.makingTime}
                          </div>
                        )}
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => saveCartItemForLater(item)}
                            style={{
                              border: 0,
                              background: "transparent",
                              color: "#365947",
            ...
            {savedForLater.length > 0 && (
              <div style={{ padding: "14px 20px", borderTop: "1px solid #e5e7eb", background: "#fffaf0" }}>
                <div style={{ fontWeight: 900, marginBottom: "10px" }}>♡ Saved for later <span style={{ color: "#64748b" }}>({savedForLater.length})</span></div>
                <div style={{ display: "grid", gap: "8px" }}>
                  {savedForLater.map((item) => (
                    <div key={item.name} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "9px", borderRadius: "12px", background: "#fff", border: "1px solid #eadfca" }}>
                      <span style={{ fontSize: "24px" }}>{item.icon}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 800, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.name}</div>
                        <div style={{ color: "#64748b", fontSize: "12px" }}>{item.price}</div>
                      </div>
                      <button type="button" onClick={() => moveSavedToCart(item)} style={{ border: 0, borderRadius: "9px", padding: "8px 10px", background: "#365947", color: "#fff", fontWeight: 800, cursor: "pointer" }}>Move to cart</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {cart.length > 0 && (
              <div style={{ padding: "14px 20px 0", borderTop: "1px solid #e5e7eb" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "9px" }}>
                  <div style={{ padding: "10px", borderRadius: "12px", background: "#f1f8f3", border: "1px solid #d5e5d9", fontSize: "11px", color: "#365947" }}>
                    🧶 <strong>Handmade</strong><br />
                    <span style={{ color: "#64748b" }}>Making time shown per item</span>
                  </div>
                  <div style={{ padding: "10px", borderRadius: "12px", background: "#f8fafc", border: "1px solid #e2e8f0", fontSize: "11px", color: "#334155" }}>
                    🔒 <strong>Fit privacy</strong><br />
                    <span style={{ color: "#64748b" }}>Photo isn't shown to seller</span>
                  </div>
                </div>

                <div style={{ marginTop: "10px", padding: "11px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
                  <div style={{ fontWeight: 900, fontSize: "12px" }}>🏷️ Coupon</div>
                  <div style={{ display: "flex", gap: "7px", marginTop: "7px" }}>
                    <input
                      value={cartCoupon}
                      onChange={(e) => setCartCoupon(e.target.value.toUpperCase())}
                      placeholder="HOWDI10"
                      style={{ flex: 1, minWidth: 0, padding: "9px", borderRadius: "9px", border: "1px solid #cbd5e1" }}
                    />
                    <button type="button" onClick={applyCartCoupon} style={{ padding: "9px 11px", border: 0, borderRadius: "9px", background: "#365947", color: "#fff", fontWeight: 900, cursor: "pointer" }}>
                      Apply
                    </button>
                  </div>
                  {cartNotice && <div style={{ marginTop: "6px", fontSize: "10.5px", color: cartCouponApplied ? "#166534" : "#64748b" }}>{cartNotice}</div>}
                </div>

                <div style={{ marginTop: "10px", padding: "11px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
                  <div style={{ fontWeight: 900, fontSize: "12px" }}>📍 Delivery pincode</div>
                  <div style={{ display: "flex", gap: "7px", marginTop: "7px" }}>
                    <input
                      value={cartPincode}
                      ...
                      style={{ flex: 1, minWidth: 0, padding: "9px", borderRadius: "9px", border: "1px solid #cbd5e1" }}
                    />
                    <button type="button" onClick={checkCartPincode} style={{ padding: "9px 11px", border: "1px solid #cbd5e1", borderRadius: "9px", background: "#fff", fontWeight: 900, cursor: "pointer" }}>
                      Check
                    </button>
                  </div>
                  {cartPincodeChecked && <div style={{ marginTop: "6px", fontSize: "10.5px", color: "#166534", fontWeight: 800 }}>✓ Pincode accepted for the frontend estimate.</div>}
                </div>

                <label style={{ display: "flex", alignItems: "center", gap: "9px", marginTop: "10px", fontSize: "12px", cursor: "pointer" }}>
                ...
                {giftWrap && (
                  ...
                  <textarea
                    ...
                    style={{ width: "100%", boxSizing: "border-box", marginTop: "7px", padding: "9px", borderRadius: "9px", border: "1px solid #cbd5e1", resize: "vertical" }}
                  />
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "8px", marginTop: "7px" }}>
                    <span style={{ fontSize: "10.5px", color: giftMessageSaved ? "#166534" : "#64748b" }}>
                      {giftMessageSaved ? "✓ Gift message saved to this cart." : "Save your message before checkout."}
                    </span>
                    <button
                      type="button"
                      onClick={() => {
                        setGiftMessageSaved(true);
                        setCartNotice("🎁 Gift message saved.");
                      }}
                      ...
                      style={{
                        ...
                        border: 0,
                        borderRadius: "9px",
                        background: giftMessage.trim() ? "#365947" : "#cbd5e1",
                        color: "#fff",
                        fontWeight: 900,
                        cursor: giftMessage.trim() ? "pointer" : "not-allowed",
                        whiteSpace: "nowrap"
                      }}
                    >
                      Save message
                ...
                <div style={{ marginTop: "10px", padding: "11px", borderRadius: "12px", background: "#fffaf0", border: "1px solid #eadfca", fontSize: "11px", color: "#684c20", lineHeight: 1.45 }}>
                  👵 <strong>Before checkout:</strong> confirm size, colour and custom-fit details. Made-to-order pieces start preparation after confirmation.
                </div>
              </div>
            )}
            ...
            {cart.length > 0 && (
              ...
              >
                ...
                >
                  <strong>Cart total preview</strong>
                ...
                >
                  {cartCouponApplied ? "Coupon discount included in preview. " : ""}{giftWrap ? "Gift wrapping included. " : ""}Final delivery charges and payment details are confirmed at checkout.
                </p>
          ...
          <section style={{ padding: "24px", margin: "0 0 10px", borderBottom: "1px solid #edf1ee", background: "#fff" }}>
            <div style={{ maxWidth: "1180px", margin: "0 auto" }}>
              ...
              >
                ...
                >
                  📍 {customerLocation || "Select your location"}
                </span>
                ...
                >
                  · {homeProducts.length} products available
                </span>
                ...
                >
                  · {homeWorkers.length} local services
                </span>
