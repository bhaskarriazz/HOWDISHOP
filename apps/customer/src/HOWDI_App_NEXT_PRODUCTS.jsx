...
const products = [
  { id: 1, icon: "🪴", name: "Indoor Plant", shop: "Green Home Store", category: "Home & Garden", price: "₹299", oldPrice: "₹399", rating: 4.8, reviews: 124, badge: "Best Seller" },
  { id: 2, icon: "💡", name: "LED Smart Light", shop: "Sri Lakshmi Electricals", category: "Electronics", price: "₹549", oldPrice: "₹699", rating: 4.7, reviews: 89, badge: "20% OFF" },
  { id: 3, icon: "🪑", name: "Wooden Stool", shop: "Local Wood Works", category: "Furniture", price: "₹899", oldPrice: "₹1,199", rating: 4.9, reviews: 67, badge: "Popular" },
  { id: 4, icon: "🍳", name: "Non-Stick Cookware Set", shop: "Home Essentials", category: "Kitchen", price: "₹1,299", oldPrice: "₹1,799", rating: 4.6, reviews: 52, badge: "Hot Deal" },
  { id: 5, icon: "🎧", name: "Wireless Earbuds", shop: "Tech Corner", category: "Electronics", price: "₹799", oldPrice: "₹1,199", rating: 4.5, reviews: 143, badge: "Trending" },
  { id: 6, icon: "🧺", name: "Storage Basket", shop: "Home Needs", category: "Home & Garden", price: "₹349", oldPrice: "₹499", rating: 4.7, reviews: 76, badge: "Value Pick" },
];
...
function App() {
  ...
  const visibleProducts = products
    ...
    .filter((product) => {
      ...
      return !query || `${product.name} ${product.shop} ${product.category}`.toLowerCase().includes(query);
    })
    .sort((a, b) => {
      if (productSort === "price-low") return Number(a.price.replace(/[^0-9]/g, "")) - Number(b.price.replace(/[^0-9]/g, ""));
      if (productSort === "price-high") return Number(b.price.replace(/[^0-9]/g, "")) - Number(a.price.replace(/[^0-9]/g, ""));
    ...
    <div className="howdi-app">
      ...
      <main>
        ...
        <section className="section shop-section" id="shop">
          <div className="section-heading">
            <div>
              <span className="section-label">SHOP • LOCAL VENDORS</span>
              <h2>Made nearby.<br />Chosen by you.</h2>
              <p>Discover products from local businesses with transparent prices, offers and easy checkout.</p>
            </div>
            <button className="outline-button" type="button" onClick={() => { setProductSearch(""); setProductCategory("All"); document.getElementById("shop")?.scrollIntoView({ behavior: "smooth" }); }}>
              View all products →
            </button>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "minmax(220px, 1fr) auto", gap: "12px", marginBottom: "18px" }}>
            <div style={{ position: "relative" }}>
              <span style={{ position: "absolute", left: "14px", top: "12px", fontSize: "18px" }}>🔎</span>
              <input value={productSearch} onChange={(e) => setProductSearch(e.target.value)} placeholder="Search products, shops or categories..." style={{ width: "100%", boxSizing: "border-box", height: "46px", padding: "0 16px 0 42px", border: "1px solid #d8d5cb", borderRadius: "12px", fontSize: "14px", outline: "none" }} />
            </div>
            <select value={productSort} onChange={(e) => setProductSort(e.target.value)} style={{ height: "46px", padding: "0 14px", border: "1px solid #d8d5cb", borderRadius: "12px", background: "#fff", fontWeight: 800 }}>
              <option value="featured">Featured</option><option value="rating">Top rated</option><option value="price-low">Price: Low to High</option><option value="price-high">Price: High to Low</option>
            </select>
          </div>

          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "22px" }}>
            {productCategories.map((category) => (
              <button key={category} type="button" onClick={() => setProductCategory(category)} style={{ border: productCategory === category ? "1px solid #365947" : "1px solid #ddd8ca", background: productCategory === category ? "#365947" : "#fff", color: productCategory === category ? "#fff" : "#24362d", padding: "9px 14px", borderRadius: "999px", fontWeight: 800, cursor: "pointer" }}>{category}</button>
            ))}
          </div>

          <div className="product-grid">
            {visibleProducts.map((product) => {
              const quantity = selectedQuantities[product.name] || 1;
              const save = Number(product.oldPrice.replace(/[^0-9]/g, "")) - Number(product.price.replace(/[^0-9]/g, ""));
              return (
                <article className="product-card" key={product.id}>
                  <div className="product-image" style={{ position: "relative", cursor: "pointer" }} onClick={() => setSelectedProduct(product)}>
                    {product.icon}
                    <span style={{ position: "absolute", top: "12px", left: "12px", padding: "6px 9px", borderRadius: "999px", background: "#fff", color: "#9a5b1f", fontSize: "11px", fontWeight: 900, boxShadow: "0 4px 12px rgba(0,0,0,.12)" }}>{product.badge}</span>
                    <button type="button" onClick={(e) => { e.stopPropagation(); toggleWishlist(product); }} aria-label={`Wishlist ${product.name}`} style={{ position: "absolute", top: "12px", right: "12px", border: "1px solid #172019", borderRadius: "999px", background: "#fff", width: "42px", height: "42px", fontSize: "22px", cursor: "pointer" }}>{wishlist.some((item) => item.name === product.name) ? "♥" : "♡"}</button>
                  </div>
                  <div className="product-body">
                    <span className="product-shop">{product.shop}</span>
                    <h3>{product.name}</h3>
                    <div style={{ fontSize: "13px", fontWeight: 800, color: "#6b7280", margin: "5px 0 8px" }}>⭐ {product.rating} · {product.reviews} reviews</div>
                    <div className="product-price"><strong>{product.price}</strong><del>{product.oldPrice}</del></div>
                    <div style={{ marginTop: "12px", padding: "10px 12px", borderRadius: "12px", background: "#f7f4ec", border: "1px solid #e4dece", display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontSize: "12px", fontWeight: 900, color: "#9a5b1f" }}>SAVE ₹{save}</span><div style={{ display: "flex", alignItems: "center", border: "1px solid #c9c2b1", borderRadius: "9px", overflow: "hidden", background: "#fff" }}><button type="button" onClick={() => setProductQuantity(product, quantity - 1)} style={{ width: "30px", height: "30px", border: 0, background: "#fff", cursor: "pointer" }}>−</button><strong style={{ minWidth: "28px", textAlign: "center" }}>{quantity}</strong><button type="button" onClick={() => setProductQuantity(product, quantity + 1)} style={{ width: "30px", height: "30px", border: 0, background: "#fff", cursor: "pointer" }}>+</button></div></div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginTop: "10px" }}><button type="button" onClick={() => buyNow(product, quantity)} style={{ minHeight: "44px", border: 0, borderRadius: "11px", background: "#0f172a", color: "#fff", fontWeight: 900, cursor: "pointer" }}>Buy Now</button><button type="button" onClick={() => addToCart(product, quantity)} style={{ minHeight: "44px", border: 0, borderRadius: "11px", background: "#365947", color: "#fff", fontWeight: 900, cursor: "pointer" }}>Add to Cart</button></div>
                  </div>
                </article>
              );
            })}
          </div>
          {!visibleProducts.length && <div style={{ padding: "50px 20px", textAlign: "center", background: "#faf9f5", borderRadius: "18px" }}><div style={{ fontSize: "36px" }}>🔎</div><h3>No products found</h3><p style={{ color: "#64748b" }}>Try another search or category.</p></div>}
        </section>

        {selectedProduct && (
          <div onMouseDown={(e) => e.target === e.currentTarget && setSelectedProduct(null)} style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(15,23,42,.55)", display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
            <div style={{ width: "min(680px, 100%)", background: "#fff", borderRadius: "24px", padding: "28px", boxShadow: "0 30px 80px rgba(0,0,0,.25)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: "20px" }}><div><div style={{ fontSize: "64px" }}>{selectedProduct.icon}</div><span className="product-shop">{selectedProduct.shop}</span><h2 style={{ margin: "8px 0" }}>{selectedProduct.name}</h2><p style={{ color: "#64748b" }}>{selectedProduct.category} · ⭐ {selectedProduct.rating} ({selectedProduct.reviews} reviews)</p></div><button type="button" onClick={() => setSelectedProduct(null)} style={{ width: "40px", height: "40px", borderRadius: "50%", border: "1px solid #ddd", background: "#fff", cursor: "pointer", fontSize: "20px" }}>×</button></div>
              <div style={{ marginTop: "18px", display: "flex", alignItems: "baseline", gap: "10px" }}><strong style={{ fontSize: "30px", color: "#153b2b" }}>{selectedProduct.price}</strong><del style={{ color: "#8b8b83" }}>{selectedProduct.oldPrice}</del></div>
              <p style={{ color: "#475569", lineHeight: 1.6 }}>Quality local product with transparent offer pricing. Select your quantity and add it to the cart or buy it now.</p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}><button type="button" onClick={() => buyNow(selectedProduct, selectedQuantities[selectedProduct.name] || 1)} style={{ minHeight: "48px", border: 0, borderRadius: "12px", background: "#0f172a", color: "#fff", fontWeight: 900, cursor: "pointer" }}>Buy Now</button><button type="button" onClick={() => addToCart(selectedProduct, selectedQuantities[selectedProduct.name] || 1)} style={{ minHeight: "48px", border: 0, borderRadius: "12px", background: "#365947", color: "#fff", fontWeight: 900, cursor: "pointer" }}>Add to Cart</button></div>
            </div>
          </div>
        )}
