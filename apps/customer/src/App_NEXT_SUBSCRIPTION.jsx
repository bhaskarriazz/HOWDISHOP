                ...
                {profileTab === "learning" && (
                  <div style={{ minHeight: "420px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "18px", flexWrap: "wrap", marginBottom: "22px" }}>
                      ...
                      <button type="button" onClick={() => setLearningNotice("More HOWDI courses will be added soon.")} style={{ border: 0, borderRadius: "10px", padding: "10px 15px", background: "#365947", color: "#fff", fontWeight: 900, cursor: "pointer" }}>Browse courses →</button>
                    </div>
                  ...
                  <div style={{ minHeight: "420px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "18px", flexWrap: "wrap", marginBottom: "22px" }}>
                      <div>
                        ...
                        <p style={{ color: "#64748b", margin: 0 }}>Get more value from HOWDI with plans built for everyday users and growing needs.</p>
                      </div>
                      <div style={{ padding: "9px 12px", borderRadius: "999px", background: subscriptionPlan === "free" ? "#f1f5f9" : "#ecfdf3", color: subscriptionPlan === "free" ? "#475569" : "#166534", fontSize: "12px", fontWeight: 900 }}>
                        {subscriptionPlan === "free" ? "FREE PLAN" : subscriptionPlan === "plus" ? "PLUS ACTIVE" : "PRO ACTIVE"}
                      </div>
                    </div>

                    {subscriptionNotice && (
                      <div style={{ marginBottom: "14px", padding: "11px 13px", borderRadius: "11px", background: "#ecfdf3", color: "#166534", fontSize: "13px", fontWeight: 800 }}>{subscriptionNotice}</div>
                    )}

                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "12px", marginBottom: "18px" }}>
                      {[
                        { id: "free", name: "HOWDI Free", price: "₹0", period: "Forever", icon: "🌱", features: ["Browse local services", "Shop local products", "Basic rewards"] },
                        { id: "plus", name: "HOWDI Plus", price: "₹99", period: "per month", icon: "⭐", features: ["Extra cashback offers", "Priority support", "Member-only deals"] },
                        { id: "pro", name: "HOWDI Pro", price: "₹199", period: "per month", icon: "👑", features: ["Higher rewards", "Premium offers", "Priority access"] },
                      ].map((plan) => {
                        ...
                        return (
                          <div key={plan.id} style={{ position: "relative", border: active ? "2px solid #365947" : "1px solid #e2e8f0", borderRadius: "18px", padding: "18px", background: active ? "#f8faf8" : "#fff", boxShadow: active ? "0 8px 24px rgba(54,89,71,.08)" : "none" }}>
                            ...
                            <div style={{ display: "flex", alignItems: "baseline", gap: "6px" }}><strong style={{ fontSize: "27px" }}>{plan.price}</strong><span style={{ color: "#64748b", fontSize: "12px" }}>{plan.period}</span></div>
                            <div style={{ display: "grid", gap: "8px", margin: "16px 0" }}>
                              {plan.features.map((feature) => <div key={feature} style={{ color: "#475569", fontSize: "13px" }}>✓ {feature}</div>)}
                            </div>
                            <button type="button" onClick={() => changeSubscriptionPlan(plan.id)} disabled={active} style={{ width: "100%", border: active ? "1px solid #b7cdbf" : 0, borderRadius: "10px", padding: "10px 12px", background: active ? "#fff" : "#365947", color: active ? "#365947" : "#fff", fontWeight: 900, cursor: active ? "default" : "pointer" }}>{active ? "Current plan" : plan.id === "free" ? "Switch to Free" : "Choose plan →"}</button>
                          </div>
                        );
                      })}
                    </div>

                    <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1.2fr) minmax(260px, .8fr)", gap: "12px" }}>
                      <div style={{ border: "1px solid #e2e8f0", borderRadius: "18px", padding: "18px", background: "#fff" }}>
                        ...
                        <div style={{ display: "grid", gap: "10px", marginTop: "14px", color: "#64748b", fontSize: "13px" }}>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", gap: "10px" }}><span>Payment</span><strong style={{ color: "#0f172a" }}>{subscriptionPlan === "free" ? "No payment required" : "Saved payment method"}</strong></div>
                        </div>
                      </div>
                      <div style={{ border: "1px solid #e2e8f0", borderRadius: "18px", padding: "18px", background: "#fff" }}>
                        <div style={{ fontWeight: 900, fontSize: "17px" }}>🔄 Auto-renew</div>
                        <p style={{ color: "#64748b", fontSize: "13px", lineHeight: 1.5, margin: "8px 0 14px" }}>Keep your paid plan active automatically at renewal.</p>
                        <button type="button" onClick={toggleSubscriptionAutoRenew} disabled={subscriptionPlan === "free"} style={{ width: "100%", border: "1px solid #dbe3dc", borderRadius: "10px", padding: "10px 12px", background: subscriptionPlan === "free" ? "#f1f5f9" : subscriptionAutoRenew ? "#365947" : "#fff", color: subscriptionPlan === "free" ? "#94a3b8" : subscriptionAutoRenew ? "#fff" : "#334155", fontWeight: 900, cursor: subscriptionPlan === "free" ? "not-allowed" : "pointer" }}>{subscriptionPlan === "free" ? "Available on paid plans" : subscriptionAutoRenew ? "✓ Auto-renew on" : "Auto-renew off"}</button>
                      </div>
                    </div>
                  </div>
                )}
